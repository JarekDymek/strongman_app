...
// *****************
// Utility: prune store to max entries to avoid unbounded growth on devices with limited storage.
// Removes oldest entries (by natural key order) until store size <= maxEntries.
// Usage: await pruneStoreIfNeeded(STORES.EVENTS, 2000);
export async function pruneStoreIfNeeded(storeName, maxEntries = 2000) {
  return new Promise((resolve, reject) => {
    ...
    openReq.onsuccess = function(evt) {
      ...
      countReq.onsuccess = function() {
        ...
        if (total <= maxEntries) {
          resolve({pruned:0, total});
          return;
        }
        const toDelete = total - maxEntries;
        let deleted = 0;
        // delete oldest by opening a cursor in ascending order
        ...
        cursorReq.onsuccess = function(e) {
          ...
          if (cursor && deleted < toDelete) {
            ...
            resolve({pruned: deleted, total});
          }
        };
        cursorReq.onerror = function(err) { console.error('prune cursor error', err); resolve({pruned: deleted, total}); };
      };
      countReq.onerror = function(err) { console.error('count error', err); resolve({pruned:0, total:0}); };
    };
    openReq.onerror = function(e) { console.error('DB open error in prune', e); resolve({pruned:0, total:0}); };
  });
}
