...
let rafId = null, tickInterval = 100, lastUiUpdate = 0, startTime, elapsedTime = 0, isRunning = false, currentCompetitor, mode, repCount = 0, lapTimes = [], onSaveCallback;
...
function startTicker() {
  // Use requestAnimationFrame for smoother updates and lower CPU on mobile devices.
  lastUiUpdate = 0;
  function tick() {
    if (!isRunning) return;
    const now = Date.now();
    // update elapsed time continuously, but throttle UI updates to tickInterval ms
    elapsedTime = now - startTime;
    if (now - lastUiUpdate >= tickInterval) {
      updateDisplay();
      lastUiUpdate = now;
    }
...
function resetStopwatch() {
  if(isRunning) { stopTicker(); isRunning = false; }
...
function handleModeAction() {
  ...
  if (mode === 'reps') {
      ...
      lapTimes.push(currentLap); if (lapTimes.length > 500) { lapTimes.shift(); }
...
function exitStopwatch() {
  stopTicker();
  try { document.removeEventListener('fullscreenchange', _stopwatch_fullscreenchange_handler); } catch(e){}
...
export function setupStopwatchEventListeners() {
    ...
    function _stopwatch_fullscreenchange_handler() {
    if (!document.fullscreenElement && stopwatchDOMElements.stopwatchEl.classList.contains('visible')) {
        exitStopwatch();
    }
}
document.removeEventListener('fullscreenchange', _stopwatch_fullscreenchange_handler); // ensure not duplicated
document.addEventListener('fullscreenchange', _stopwatch_fullscreenchange_handler);
}
